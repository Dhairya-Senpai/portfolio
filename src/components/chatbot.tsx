...
export default function ChatBot() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // For GH Pages (no backend): Simple mock response
      // In production, you'd call your own backend endpoint instead
      const mockResponse = `Thanks for your question about "${userMessage}". Since this is a static portfolio on GitHub Pages, I can't process complex queries. However, you can:\n\n• Email me at dmeshram@umd.edu\n• Visit my GitHub: github.com/Dhairya-Senpai\n• Connect on LinkedIn: linkedin.com/in/dhairyashil-meshram`
      
      setMessages((prev) => [...prev, { role: 'assistant', content: mockResponse }])
    } catch (error) {
      console.error('Chat error:', error)
      setMessages((prev) => [...prev, { role: 'assistant', content: 'Sorry, I encountered an error. Please try again later or reach out via email.' }])
    } finally {
      setIsLoading(false)
    }
  }
